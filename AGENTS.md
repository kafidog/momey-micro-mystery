...
Momey Micro-Mystery A9R4 static multiplayer frontend + Cloudflare Worker/Durable Object A9R4Room.

## Sources of truth and startup

Read global/repo AGENTS → existing delivery contract → `playable-a9r4/CODEX_HANDOFF.md` → selected GitHub Issue / explicit user task → `git branch --show-current`, `git rev-parse HEAD`, `git status --short`, `git diff` (and staged diff). This repository has no standalone PROJECT_DELIVERY_RULES.md; retain the specific rules below and in the current handoff instead of inventing a duplicate.

Only `playable-a9r4/CODEX_HANDOFF.md` is current; older milestone handoffs are historical, not competing current entries. Supporting sources: `playable-a9r4/README.md`, `worker-a9r4/package.json`.
...
Use worker-a9r4/package.json scripts: npm --prefix worker-a9r4 test; targeted test:outcome; integration test:local and test:frontend. Frontend is static; do not invent a frontend build script. Runtime deployment needs separate authorization.

Cloudflare runtime credentials and actual human/physical-device evidence are separate gates. Browser multi-device tests do not prove human fun, WTP or physical iOS/Android.
