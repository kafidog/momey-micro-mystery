(() => {
  ...
  function renderOwnEvidence(roleId, seat) {
    ...
    node.hidden = false;
    node.innerHTML = `<div class="panel-label">STAGE 03｜角色行動結果</div>
  ...
  function contributionPrompts() {
    return `<div class="prompt-grid" aria-label="公開交換的自然問題">
      <div><strong>事件指揮</strong><p>你的時間線發現了什麼衝突？</p></div>
  ...
  function renderSharingStage(seat) {
    if (!seat.actionComplete) {
      return `<section class="stage-block sharing-stage" aria-labelledby="sharing-title"><div class="stage-title"><span>STAGE 04</span><h2 id="sharing-title">公開交換</h2></div><p class="stage-lock">先完成這支手機的角色工具，再把你帶回的證據說給另外兩位隊友。</p></section>`;
    }
    if (!seat.teamReady) {
      return `<section class="stage-block sharing-stage" aria-labelledby="sharing-title"><div class="stage-title"><span>STAGE 04</span><h2 id="sharing-title">公開交換</h2></div>
        <p class="discussion-question">把自己的證據說完，也聽完隊友的三段說明。可以隨時回到上方「重新查看我的證據」。</p>
        ${contributionPrompts()}
        <button class="primary-button confirm-button" type="button" data-team-ready>三個人的結果都已經說完了</button>
        <p class="stage-lock">這個確認代表隊伍已經完成口頭交換，接著只選一個未知查證。</p></section>`;
    }
    return `<section class="stage-block sharing-stage is-ready" aria-labelledby="sharing-title"><div class="stage-title"><span>STAGE 04</span><h2 id="sharing-title">公開交換完成</h2></div>
      <p class="discussion-question">三個人的結果都已經說完了。需要細節時，由原本的角色重新查看自己的證據。</p>
      ${compactTeamBoard()}</section>`;
  }

  function renderEscalationStage() {
    return `<section class="stage-block escalation-stage" aria-labelledby="escalation-title"><div class="stage-title"><span>STAGE 05</span><h2 id="escalation-title">事故升級</h2></div>
      <p>共同時間線停在 18:48。西側人的現況／路線和東閘撐點承載，仍是兩個不同的未知；剩餘電容只能查一端。</p>
      <div class="unknown-pair"><div><span>未知一</span><strong>林芮的即時性與西側路線</strong><small>回應可信，但是不是現在傳送、路線能否走完 95 秒仍未知。</small></div><div><span>未知二</span><strong>東閘撐點的承載</strong><small>95 秒會把壓力帶到東閘，但撐架能否撐住仍未知。</small></div></div></section>`;
  }
  ...
  function renderVerification(shared, ready) {
    ...
    return `<section class="stage-block verification-stage" aria-labelledby="verification-title"><div class="stage-title"><span>STAGE 06</span><h2 id="verification-title">有限查證</h2></div>
      <p class="stage-lede">剩餘電容只能打一個高能脈衝。A／B 各自打開一個未知；確認前可以換，確認後只留一個結果。</p>
      ...
      ${!ready ? `<p class="stage-lock">先完成公開交換確認，再從兩個未知中選一個查證。</p>` : ""}${result}</section>`;
  }

  function renderDecision(shared, ready) {
    if (!ready || !shared.verificationConfirmed) {
      return `<section class="stage-block locked-decision"><div class="stage-title"><span>STAGE 08</span><h2>共同決定</h2></div><p>先完成一次有限查證，再把「立即封閉」和「維持 95 秒」放在同一張桌上。</p></section>`;
    }
  ...
  function renderTeamStages() {
    ...
    target.innerHTML = `<div class="team-flow">${renderSharingStage(seat)}${seat.teamReady ? renderEscalationStage() : ""}${renderVerification(shared, ready)}${renderDecision(shared, ready)}${renderConsequence(shared, ready)}<div class="team-reset"><button class="text-button" type="button" data-reset>清除本次 A3 進度</button></div></div>`;
  }
  ...
  function resetPrototype() {
    ...
    if (store) {
      try {
        ...
        // Ignore storage errors and return to the A3 entry surface.
      }
    }
  ...
  function handleClick(event) {
    ...
    if (teamReady && !teamReady.disabled && currentRole) {
      ...
      if (seat.actionComplete && !seat.teamReady) {
        writeSeat(currentRole, { ...seat, teamReady: true, teamReadyAt: "18:48，完成公開交換確認" });
  ...
  function init() {
    ...
    window.MomeyA3 = { readSeat, readShared, currentSessionReady, resetPrototype };
  }
