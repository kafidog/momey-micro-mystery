...
test("A9R4 is an isolated SQLite Durable Object with WebSocket Hibernation", () => {
  assert.match(config, /name = "momey-playable-a9r4-room"/);
  assert.match(config, /name = "ROOM_A9R4"/);
  assert.match(config, /class_name = "A9R4Room"/);
  assert.match(config, /new_sqlite_classes = \["A9R4Room"\]/);
  assert.match(source, /class A9R4Room extends DurableObject/);
  assert.match(source, /A9R4_SCHEMA/);
  assert.doesNotMatch(source, /A9R3|ROOM_A9R3|a9r3/i);
...
test("A9R4 room surface remains title-specific and CORS-bounded", () => {
