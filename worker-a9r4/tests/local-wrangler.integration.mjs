...
const PORT = Number(process.env.MOMEY_A9R4_TEST_PORT || 8790);
const REMOTE = String(process.env.MOMEY_A9R4_REMOTE_URL || "").replace(/\/$/, "");
const BASE = REMOTE || `http://127.0.0.1:${PORT}`;
const WS_BASE = BASE.replace(/^http:/i, "ws:").replace(/^https:/i, "wss:");
const ORIGIN = process.env.MOMEY_A9R4_TEST_ORIGIN || (REMOTE ? "https://kafidog.github.io" : "http://127.0.0.1:1590");
...
async function run() {
  ...
  assert.equal(groupA[0].state.phase, "BRIEFING");
  const introMasterRole = groupA[0].state.audioMasterRole;
  const introMaster = groupA.find((client) => client.role === introMasterRole);
  for (let beat = 1; beat <= 6; beat += 1) { await converge(groupA, `briefing ${beat}`); await introMaster.command("COMPLETE_OPERATOR", { eventId: `A9R_BRIEFING_${beat}` }); }
  ...
  assert.equal(groupA[0].state.training.complete, true);
  await introMaster.command("COMPLETE_OPERATOR", { eventId: "A9R_TRAINING_COMPLETE" });
  await converge(groupA, "training complete");
  ...
  assert.equal(groupA[2].state.control.braceActive, true);
  assert.equal(groupA[0].state.control.safetySupportCoarse, undefined);
  ...
  assert.ok(groupA[1].state.control.positionExact > 0);
  assert.equal(groupA[0].state.shared.trolley, undefined);
  assert.equal(groupA[0].state.control.checkpointExact, undefined);

  const rescueToken = groupA[1].token;
  groupA[1].close();
  await groupA[0].wait(() => /安全放開/.test(groupA[0].state.shared.systemEvent), "disconnect neutral");
  ...
  assert.equal(groupB[0].state.shared.systemEvent, "三支手機已連上同一場事件。");
  assert.equal(groupB[0].state.shared.trolley, undefined);
  console.log(`A9R4 realtime PASS: mode=${REMOTE ? "remote" : "local"} rooms=${roomA},${roomB} clients=3+3 briefing=6 training-links=3 live-cross-links=3 projection-leak=PASS reconnect=PASS cross-room=PASS`);
}
