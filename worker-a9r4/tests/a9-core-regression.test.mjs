...
function comparableMetrics(state) {
  ...
  delete m.prematureCloseCount;
  m.lastAt -= state.phaseStartedAt;
  return m;
}

test("A9R4 preserves the A9R3 six briefing beats and ordered cooperative training", () => {
  const ctx = enterWindow1(a9r4, "BRF234");
  ...
  assert.equal(a9r4.projectRoomState(ctx.state, "operations", ctx.state.metrics.lastAt).sharedObjective, "把林芮送到第二標記，同時別讓閘門進入危急。");
});

test("A9R4 physics remains identical to the A9R3 formulas", () => {
  const base = enterWindow1(a9r3, "FORM34");
  const next = enterWindow1(a9r4, "FORM34");
  for (const [role, type, extra] of [
    ["operations", "SET_POWER", { mode: "rail" }],
    ["rescue", "TROLLEY_START", {}],
    ["safety", "BRACE_START", {}]
  ]) {
    base.state = issue(a9r3, base.state, base.tokens, role, type, extra);
    next.state = issue(a9r4, next.state, next.tokens, role, type, extra);
  }
  base.state = a9r3.materialize(base.state, base.state.metrics.lastAt + 10_000);
  next.state = a9r4.materialize(next.state, next.state.metrics.lastAt + 10_000);
...
test("A9R4 preserves A9R3 event-driven Window 1 timing and route lock", () => {
  const base = enterWindow1(a9r3, "PACE34");
  const next = enterWindow1(a9r4, "PACE34");
  for (const ctx of [base, next]) {
    const engine = ctx === base ? a9r3 : a9r4;
  ...
  assert.equal(base.state.phase, "INTERLUDE", "A9R3 advances when the objective is complete");
  assert.equal(next.state.phase, "INTERLUDE", "A9R4 preserves event-driven completion");
