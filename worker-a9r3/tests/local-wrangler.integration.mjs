...
const PORT = Number(process.env.MOMEY_A9R3_TEST_PORT || 8790);
const REMOTE = String(process.env.MOMEY_A9R3_REMOTE_URL || "").replace(/\/$/, "");
const BASE = REMOTE || `http://127.0.0.1:${PORT}`;
const WS_BASE = BASE.replace(/^http:/i, "ws:").replace(/^https:/i, "wss:");
const ORIGIN = process.env.MOMEY_A9R3_TEST_ORIGIN || (REMOTE ? "https://kafidog.github.io" : "http://127.0.0.1:1590");
...
async function run() {
  ...
  console.log(`A9R3 realtime PASS: mode=${REMOTE ? "remote" : "local"} rooms=${roomA},${roomB} clients=3+3 briefing=6 training-links=3 live-cross-links=3 projection-leak=PASS reconnect=PASS cross-room=PASS`);
}
