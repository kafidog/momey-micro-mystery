...
test("200-run A9R2 and A9R3 local-only versus coordinated policy attack", () => {
  evidence.a9r2Silent = distribution(a9r2, false, 200, 1000);
  evidence.a9r3Silent = distribution(a9r3, false, 200, 2000);
  evidence.a9r3Coordinated = distribution(a9r3, true, 200, 3000);
  evidence.delta = evidence.a9r3Coordinated.successRate - evidence.a9r3Silent.successRate;
  assert.ok(evidence.a9r2Silent.successRate >= 0.60, "A9R2 baseline must reproduce reliable silent success");
  // Final A9R3 acceptance is intentionally strict. This assertion demonstrates whether
  // information boundaries alone are sufficient or escalation is required.
  assert.ok(evidence.a9r3Silent.successRate <= 0.55, `A9R3 silent success still too reliable: ${evidence.a9r3Silent.successRate}`);
  assert.ok(evidence.delta >= 0.25, `coordinated delta too small: ${evidence.delta}`);
});

test.after(async () => {
  if (process.env.MOMEY_A9R3_POLICY_EVIDENCE) await writeFile(process.env.MOMEY_A9R3_POLICY_EVIDENCE, JSON.stringify(evidence, null, 2) + "\n", "utf8");
});
