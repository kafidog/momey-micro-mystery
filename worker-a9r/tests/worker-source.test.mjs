...
test("A9R is an isolated SQLite Durable Object with WebSocket Hibernation", () => {
  assert.match(config, /name = "momey-playable-a9r-room"/);
  assert.match(config, /name = "ROOM_A9R"/);
  assert.match(config, /class_name = "A9RRoom"/);
  assert.match(config, /new_sqlite_classes = \["A9RRoom"\]/);
  assert.match(source, /class A9RRoom extends DurableObject/);
...
test("A9R room surface remains title-specific and CORS-bounded", () => {
