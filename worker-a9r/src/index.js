...
const STORAGE_TABLE = "a9r_room_state";
...
export class A9RRoom extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx = ctx;
    this.env = env;
  ...
  saveState(state) {
    this.ctx.storage.sql.exec(
      ...
      JSON.stringify(state), A9R_SCHEMA, Date.now()
    );
  }
...
async function initRoom(env, roomCode) {
  const id = env.ROOM_A9R.idFromName(`a9r:${roomCode}`);
  const response = await env.ROOM_A9R.get(id).fetch(`https://a9r-room.internal/init?roomCode=${roomCode}`, { method: "POST" });
  return response.json();
}

function roomFetch(env, roomCode, path, request = null) {
  const id = env.ROOM_A9R.idFromName(`a9r:${roomCode}`);
  const target = new URL(`https://a9r-room.internal${path}`);
  return env.ROOM_A9R.get(id).fetch(request ? new Request(target, request) : new Request(target));
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health" && request.method === "GET") return jsonResponse(request, { service: "momey-playable-a9r-room", schema: A9R_SCHEMA, status: "ok" });
    if (url.pathname === "/rooms" && request.method === "POST") {
      const allocation = await findAvailableRoomCode(rngFromCrypto(), (roomCode) => initRoom(env, roomCode));
      return allocation ? jsonResponse(request, { roomCode: allocation.roomCode, sharePath: `/playable-a9r/?room=${allocation.roomCode}` }, 201) : jsonResponse(request, { error: "ROOM_CODE_UNAVAILABLE" }, 503);
    }
