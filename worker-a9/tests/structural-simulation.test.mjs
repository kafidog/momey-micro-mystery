...
function startWindow1() {
  ...
  state = command(state, tokens, state.audioMasterRole, "COMPLETE_OPERATOR", { eventId: "A9_INTRO" });
...
function readyFinalState(overrides = {}) {
  ...
  ctx.state.phaseStartedAt = ctx.state.metrics.lastAt;
...
test("four complete Council-perspective games reject the A8 form loop", () => {
  const coordinated = coordinatedGame("G1 Coordinated baseline", { operations: "Rebutter", rescue: "Opportunity Finder", safety: "Outsider" });
  const silent = silentLocalOnlyGame();
  const recovered = recoveredMistakeGame();
  const retreat = retreatTradeoffGame();
  assert.equal(coordinated.outcome, "COORDINATED_CLOSE");
  assert.ok(silent.damage > coordinated.damage || silent.exposure > coordinated.exposure || silent.outcome !== coordinated.outcome, "silent local-only policy must materially underperform coordinated play");
  assert.equal(recovered.gateClosed, true);
  assert.equal(retreat.outcome, "RESCUE_WITH_GATE_DAMAGE");
...
test.after(async () => {
  if (process.env.MOMEY_A9_EVIDENCE_PATH) {
    await writeFile(process.env.MOMEY_A9_EVIDENCE_PATH, JSON.stringify(evidence, null, 2) + "\n", "utf8");
  }
});
