...
const STORAGE_TABLE = "a9_room_state";
...
export class A9Room extends DurableObject {
  constructor(ctx, env) {
    super(ctx, env);
    this.ctx = ctx;
    this.env = env;
  ...
  saveState(state) {
    this.ctx.storage.sql.exec(
      ...
      JSON.stringify(state), A9_SCHEMA, Date.now()
    );
  }
...
async function initRoom(env, roomCode) {
  const id = env.ROOM.idFromName(`a9:${roomCode}`);
  const response = await env.ROOM.get(id).fetch(`https://a9-room.internal/init?roomCode=${roomCode}`, { method: "POST" });
  return response.json();
}

function roomFetch(env, roomCode, path, request = null) {
  const id = env.ROOM.idFromName(`a9:${roomCode}`);
  const target = new URL(`https://a9-room.internal${path}`);
  return env.ROOM.get(id).fetch(request ? new Request(target, request) : new Request(target));
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/health" && request.method === "GET") return jsonResponse(request, { service: "momey-playable-a9-room", status: "ok" });
    if (url.pathname === "/rooms" && request.method === "POST") {
      const allocation = await findAvailableRoomCode(rngFromCrypto(), (roomCode) => initRoom(env, roomCode));
      return allocation ? jsonResponse(request, { roomCode: allocation.roomCode, sharePath: `/playable-a9/?room=${allocation.roomCode}` }, 201) : jsonResponse(request, { error: "ROOM_CODE_UNAVAILABLE" }, 503);
    }
