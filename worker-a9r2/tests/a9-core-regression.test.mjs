...
import * as a9r from "../../worker-a9r/src/engine.js";
import * as a9r2 from "../src/engine.js";
...
test("A9R2 preserves the six briefing beats and ordered cooperative training", () => {
  const ctx = enterWindow1(a9r2, "BRF234");
  ...
  assert.equal(a9r2.projectRoomState(ctx.state, "operations", ctx.state.metrics.lastAt).phaseGoal.items.length, 2);
});

test("A9R2 physics remains identical to A9R before the new objective threshold", () => {
  const base = enterWindow1(a9r, "A9R234");
  const next = enterWindow1(a9r2, "A92234");
  for (const [role, type, extra] of [
    ["operations", "SET_POWER", { mode: "rail" }],
    ["rescue", "TROLLEY_START", {}],
    ["safety", "BRACE_START", {}]
  ]) {
    base.state = issue(a9r, base.state, base.tokens, role, type, extra);
    next.state = issue(a9r2, next.state, next.tokens, role, type, extra);
  }
  base.state = a9r.materialize(base.state, base.state.metrics.lastAt + 10_000);
  next.state = a9r2.materialize(next.state, next.state.metrics.lastAt + 10_000);
...
test("A9R2 changes only phase timing and visible route-lock state at the Window 1 goal", () => {
  const base = enterWindow1(a9r, "OLD234");
  const next = enterWindow1(a9r2, "NEW234");
  for (const ctx of [base, next]) {
    const engine = ctx === base ? a9r : a9r2;
  ...
  assert.equal(base.state.phase, "WINDOW1", "A9R still waits for its fixed deadline");
  assert.equal(next.state.phase, "INTERLUDE", "A9R2 advances when the objective is complete");
