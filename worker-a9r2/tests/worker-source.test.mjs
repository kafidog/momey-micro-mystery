...
test("A9R2 is an isolated SQLite Durable Object with WebSocket Hibernation", () => {
  assert.match(config, /name = "momey-playable-a9r2-room"/);
  assert.match(config, /name = "ROOM_A9R2"/);
  assert.match(config, /class_name = "A9R2Room"/);
  assert.match(config, /new_sqlite_classes = \["A9R2Room"\]/);
  assert.match(source, /class A9R2Room extends DurableObject/);
...
test("A9R2 room surface remains title-specific and CORS-bounded", () => {
