(function (global) {
  ...
  var DIALOGUE = global.MOMEY_A9R2_DIALOGUE || {};
  ...
  var workerBaseUrl = String(query.get("worker") || "https://momey-playable-a9r2-room.momey-micro-mystery.workers.dev").replace(/\/$/, "");
  ...
  var clockOffset = 0;
  var lastMajorEventSerial = null;

  function escapeHtml(value) {
    return String(value == null ? "" : value).replace(/[&<>'"]/g, function (char) { return ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", "'": "&#39;", '"': "&quot;" })[char]; });
  }

  function storageKey(name) { return "momey-a9r2:" + (roomCode || "none") + ":" + name; }
  ...
  function commonLiveHead() {
    var serial = Number(state.shared.majorEventSerial || 0);
    var isNew = lastMajorEventSerial !== null && serial !== lastMajorEventSerial;
    lastMajorEventSerial = serial;
    return '<div class="phase-head"><div class="phase-row"><div><p class="eyebrow">' + escapeHtml(PHASE_LABELS[state.phase]) + '</p><strong>' + escapeHtml(ROLE_LABELS[currentRole]) + '</strong></div><div class="timer" data-timer>' + escapeHtml(state.shared.timerLabel || "") + "</div></div></div>" + goalMarkup() + '<div class="coarse"><div><small>救援車</small><strong>' + escapeHtml(state.shared.trolley) + '</strong></div><div><small>閘門</small><strong>' + escapeHtml(state.shared.pressure) + '</strong></div><div><small>電力</small><strong>' + escapeHtml(state.shared.power) + '</strong></div></div><div class="event-strip' + (isNew ? " is-new" : "") + '" data-major-event>' + escapeHtml(state.shared.latestMajorEvent) + "</div>" + voiceMarkup(true);
  }

  function goalMarkup() {
    var goal = state?.phaseGoal;
    if (!goal) return "";
    return '<section class="shared-goal" data-shared-goal><strong>' + escapeHtml(goal.title) + '</strong><div class="goal-items">' + goal.items.map(function (item) { return '<span class="goal-item ' + (item.done ? "is-done" : "") + '" data-goal-id="' + escapeHtml(item.id) + '"><b aria-hidden="true">' + (item.done ? "✓" : "○") + "</b>" + escapeHtml(item.label) + "</span>"; }).join("") + "</div></section>";
  }
  ...
  function liveMarkup() {
    ...
    return '<section class="stage" data-view="live" data-phase="' + state.phase + '">' + commonLiveHead() + instrument + '<details class="live-help"><summary>查看本席說明</summary><p>' + escapeHtml(objective) + "</p></details>" + statusMarkup() + "</section>";
  }
  ...
  function outcomeMarkup() {
    var outcome = state.outcome || { causal: [], recap: [] };
    return '<section class="stage" data-view="outcome"><p class="eyebrow">事件結果</p><h1>這次操作已結束。</h1>' + voiceMarkup() + '<div class="panel causal-recap"><h2>你們怎麼把事件推到這裡</h2><ol class="outcome-list">' + (outcome.recap || []).map(function (row) { return "<li>" + escapeHtml(row) + "</li>"; }).join("") + '</ol></div><details class="metric-recap"><summary>查看控制數值</summary><ol class="outcome-list">' + (outcome.causal || []).map(function (row) { return "<li>" + escapeHtml(row) + "</li>"; }).join("") + '</ol></details><button class="secondary" type="button" data-new-event>重新建立事件</button>' + statusMarkup() + "</section>";
  }
