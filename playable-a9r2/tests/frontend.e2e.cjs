...
const WORKER_ROOT = path.resolve(__dirname, "..", "..", "worker-a9r2");
const STATIC_PORT = Number(process.env.MOMEY_A9R2_STATIC_PORT || 1590);
const WORKER_PORT = Number(process.env.MOMEY_A9R2_BROWSER_WORKER_PORT || 8790);
const REMOTE_FRONTEND = String(process.env.MOMEY_A9R2_FRONTEND_URL || "").replace(/\/$/, "");
const REMOTE_WORKER = String(process.env.MOMEY_A9R2_REMOTE_URL || "").replace(/\/$/, "");
const FRONTEND = REMOTE_FRONTEND || `http://127.0.0.1:${STATIC_PORT}`;
const WORKER = REMOTE_WORKER || `http://127.0.0.1:${WORKER_PORT}`;
const SCREENSHOTS = process.env.MOMEY_A9R2_SCREENSHOT_DIR || "";
const EVIDENCE = process.env.MOMEY_A9R2_FRONTEND_EVIDENCE || "";
const NPX_CLI = path.join(path.dirname(process.execPath), "node_modules", "npm", "bin", "npx-cli.js");
const BROWSER_EXECUTABLE = process.env.MOMEY_A9R2_BROWSER || "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe";
let staticServer;
let wrangler;
...
async function startServices() {
  ...
  throw new Error("A9R2 service health timeout");
}
...
async function phase(page, expected, timeout = 20000) {
  await page.waitForFunction((value) => window.__MOMEY_A9R2__?.getState()?.phase === value, expected, { timeout });
}
...
async function clickPower(page, mode) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R2__?.getState()?.control?.powerMode === value, mode, { timeout: REMOTE_WORKER ? 15000 : 5000 });
}
...
async function selectSeat(page, role) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R2__?.getCurrentRole() === value && window.__MOMEY_A9R2__?.getState()?.currentSeat?.started, role, { timeout: 10000 });
}
...
async function browserRun() {
  ...
  try {
    ...
    for (const beat of [1, 2, 3, 5, 6]) {
      await operations.waitForFunction((value) => window.__MOMEY_A9R2__?.getState()?.briefing?.beat === value, beat, { timeout: 20000 });
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R2__?.getState()?.training?.effects?.railPower === true);
    await snapshot(rescue, "09_training_rescue_receives_power.png");
    await train(rescue);
    await safety.waitForFunction(() => window.__MOMEY_A9R2__?.getState()?.training?.effects?.gateLoad === true);
    await snapshot(safety, "10_training_safety_sees_pressure.png");
    await train(safety);
    await operations.waitForFunction(() => window.__MOMEY_A9R2__?.getState()?.training?.effects?.support === true);
    ...
    const phaseBeforeReplay = await rescue.evaluate(() => window.__MOMEY_A9R2__.getState().phase);
    await rescue.locator("[data-replay]").click();
    assert.equal(await rescue.evaluate(() => window.__MOMEY_A9R2__.getState().phase), phaseBeforeReplay, "Replay must not advance shared state");
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R2__.getState().control.railPowered === false);
    await clickPower(operations, "rail");
    await rescue.waitForFunction(() => window.__MOMEY_A9R2__.getState().control.railPowered === true);
    await down(safety, "[data-hold-start=BRACE_START]");
    await operations.waitForFunction(() => window.__MOMEY_A9R2__.getState().control.safetySupportCoarse === "有人撐住");
    ...
    for (let cycle = 0; cycle < 5; cycle += 1) {
      if ((await operations.evaluate(() => window.__MOMEY_A9R2__.getState().phase)) !== "WINDOW2") break;
      await clickPower(operations, "balanced");
      await down(safety, "[data-hold-start=BRACE_START]");
      await down(rescue, "[data-hold-start=TROLLEY_START]");
      await delay(5000);
      await up(rescue); await up(safety);
      if ((await operations.evaluate(() => window.__MOMEY_A9R2__.getState().phase)) === "WINDOW2" && cycle < 4) {
        await clickPower(operations, "gate");
        await delay(7000);
      }
    }
    ...
    for (let cycle = 0; cycle < 4; cycle += 1) {
      if ((await operations.evaluate(() => window.__MOMEY_A9R2__.getState().phase)) === "OUTCOME") break;
      await down(safety, "[data-hold-start=BRACE_START]");
      await down(operations, "[data-hold-start=CLOSE_START]");
      await delay(3000);
      await up(operations); await up(safety);
      if ((await operations.evaluate(() => window.__MOMEY_A9R2__.getState().phase)) !== "OUTCOME") await delay(7000);
    }
    await Promise.all([phase(operations, "OUTCOME", 20000), phase(rescue, "OUTCOME", 20000), phase(safety, "OUTCOME", 20000)]);
    await snapshot(operations, "15_outcome_390x844.png");
    await snapshot(rescue, "16_outcome_412x915.png");
    const outcome = await operations.evaluate(() => window.__MOMEY_A9R2__.getState().outcome);
    ...
    const report = { mode: REMOTE_FRONTEND && REMOTE_WORKER ? "production" : "local", room, viewports: ["390x844", "412x915", "390x844"], synchronizedSeats: 3, briefingBeats: 6, trainingCrossPhoneLinksObserved: 3, liveCrossPhoneLinksObserved: 4, operatorCompactAfterPlayback: true, finalOutcome: outcome.variant, holdControls: ["TROLLEY", "BRACE", "CLOSE"], reconnectCoveredBy: "worker integration suite", missingAudioFallback: "PASS", horizontalOverflow: false, screenshots: SCREENSHOTS ? fs.readdirSync(SCREENSHOTS).filter((name) => name.endsWith(".png")).length : 0 };
    if (EVIDENCE) fs.writeFileSync(EVIDENCE, JSON.stringify(report, null, 2) + "\n", "utf8");
    console.log("A9R2 frontend three-device PASS " + JSON.stringify(report));
  } finally {
