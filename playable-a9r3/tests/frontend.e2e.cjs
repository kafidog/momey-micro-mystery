...
const WORKER_ROOT = path.resolve(__dirname, "..", "..", "worker-a9r3");
const STATIC_PORT = Number(process.env.MOMEY_A9R3_STATIC_PORT || 1590);
const WORKER_PORT = Number(process.env.MOMEY_A9R3_BROWSER_WORKER_PORT || 8790);
const REMOTE_FRONTEND = String(process.env.MOMEY_A9R3_FRONTEND_URL || "").replace(/\/$/, "");
const REMOTE_WORKER = String(process.env.MOMEY_A9R3_REMOTE_URL || "").replace(/\/$/, "");
const FRONTEND = REMOTE_FRONTEND || `http://127.0.0.1:${STATIC_PORT}`;
const WORKER = REMOTE_WORKER || `http://127.0.0.1:${WORKER_PORT}`;
const SCREENSHOTS = process.env.MOMEY_A9R3_SCREENSHOT_DIR || "";
const EVIDENCE = process.env.MOMEY_A9R3_FRONTEND_EVIDENCE || "";
const NPX_CLI = path.join(path.dirname(process.execPath), "node_modules", "npm", "bin", "npx-cli.js");
const BROWSER_EXECUTABLE = process.env.MOMEY_A9R3_BROWSER || "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe";
let staticServer;
let wrangler;
...
async function startServices() {
  ...
  throw new Error("A9R3 service health timeout");
}
...
async function phase(page, expected, timeout = 20000) {
  await page.waitForFunction((value) => window.__MOMEY_A9R3__?.getState()?.phase === value, expected, { timeout });
}
...
async function clickPower(page, mode) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R3__?.getState()?.control?.powerMode === value, mode, { timeout: REMOTE_WORKER ? 15000 : 5000 });
}
...
async function selectSeat(page, role) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R3__?.getCurrentRole() === value && window.__MOMEY_A9R3__?.getState()?.currentSeat?.started, role, { timeout: 10000 });
}
...
async function browserRun() {
  ...
  try {
    ...
    for (const beat of [1, 2, 3, 5, 6]) {
      await operations.waitForFunction((value) => window.__MOMEY_A9R3__?.getState()?.briefing?.beat === value, beat, { timeout: 20000 });
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R3__?.getState()?.training?.effects?.railPower === true);
    await snapshot(rescue, "09_training_rescue_receives_power.png");
    await train(rescue);
    await safety.waitForFunction(() => window.__MOMEY_A9R3__?.getState()?.training?.effects?.gateLoad === true);
    await snapshot(safety, "10_training_safety_sees_pressure.png");
    await train(safety);
    await operations.waitForFunction(() => window.__MOMEY_A9R3__?.getState()?.training?.effects?.support === true);
    ...
    const phaseBeforeReplay = await rescue.evaluate(() => window.__MOMEY_A9R3__.getState().phase);
    await rescue.locator("[data-replay]").click();
    assert.equal(await rescue.evaluate(() => window.__MOMEY_A9R3__.getState().phase), phaseBeforeReplay, "Replay must not advance shared state");
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R3__.getState().control.railPowered === true);
    await down(safety, "[data-hold-start=BRACE_START]");
    await safety.waitForFunction(() => window.__MOMEY_A9R3__.getState().control.braceActive === true);
    assert.equal(await operations.evaluate(() => window.__MOMEY_A9R3__.getState().control.safetySupportCoarse), undefined, "Operations must not receive Safety readiness");
    assert.equal(await operations.evaluate(() => window.__MOMEY_A9R3__.getState().shared.trolley), undefined, "common header must not mirror Rescue state");
    assert.equal(await operations.evaluate(() => window.__MOMEY_A9R3__.getState().shared.pressure), undefined, "common header must not mirror Safety state");
    ...
    for (let cycle = 0; cycle < 5; cycle += 1) {
      if ((await operations.evaluate(() => window.__MOMEY_A9R3__.getState().phase)) !== "WINDOW2") break;
      await clickPower(operations, "balanced");
      await down(safety, "[data-hold-start=BRACE_START]");
      await down(rescue, "[data-hold-start=TROLLEY_START]");
      await delay(5000);
      await up(rescue); await up(safety);
      if ((await operations.evaluate(() => window.__MOMEY_A9R3__.getState().phase)) === "WINDOW2" && cycle < 4) {
        await clickPower(operations, "gate");
        await delay(7000);
      }
    }
    ...
    await clickPower(operations, "gate");
    await rescue.waitForFunction(() => window.__MOMEY_A9R3__.getState().control.secured === true, null, { timeout: 10000 });
    await safety.waitForFunction(() => {
      const control = window.__MOMEY_A9R3__.getState().control;
      return control.pressureWindowExact === "現在可關閘" && control.braceStaminaExact >= 50;
    }, null, { timeout: 45000 });
    await snapshot(safety, "16_final_safety_window.png");
    await down(safety, "[data-hold-start=BRACE_START]");
    await safety.waitForFunction(() => window.__MOMEY_A9R3__.getState().control.braceStableExact === true, null, { timeout: 8000 });
    assert.equal(await operations.evaluate(() => window.__MOMEY_A9R3__.getState().control.braceStableExact), undefined, "Operations still must rely on Safety's callout");
    assert.equal(await operations.evaluate(() => window.__MOMEY_A9R3__.getState().control.secured), undefined, "Operations still must rely on Rescue's callout");
    ...
    const outcome = await operations.evaluate(() => window.__MOMEY_A9R3__.getState().outcome);
    ...
    const report = { mode: REMOTE_FRONTEND && REMOTE_WORKER ? "production" : "local", room, viewports: ["390x844", "412x915", "390x844"], synchronizedSeats: 3, briefingBeats: 6, trainingCrossPhoneLinksObserved: 3, liveCrossPhoneLinksObserved: 3, roleLocalProjectionLeakChecks: "PASS", operatorCompactAfterPlayback: true, finalOutcome: outcome.variant, holdControls: ["TROLLEY", "BRACE", "CLOSE"], reconnectCoveredBy: "worker integration suite", missingAudioFallback: "PASS", horizontalOverflow: false, screenshots: SCREENSHOTS ? fs.readdirSync(SCREENSHOTS).filter((name) => name.endsWith(".png")).length : 0 };
    if (EVIDENCE) fs.writeFileSync(EVIDENCE, JSON.stringify(report, null, 2) + "\n", "utf8");
    console.log("A9R3 frontend three-device PASS " + JSON.stringify(report));
  } finally {
