(function (global) {
  ...
  var DIALOGUE = global.MOMEY_A9R3_DIALOGUE || {};
  ...
  var workerBaseUrl = String(query.get("worker") || "https://momey-playable-a9r3-room.momey-micro-mystery.workers.dev").replace(/\/$/, "");
  ...
  function storageKey(name) { return "momey-a9r3:" + (roomCode || "none") + ":" + name; }
  ...
  function localEventMarkup(control) {
    ...
    return '<div class="event-strip local-event' + (isNew ? " is-new" : "") + '" data-local-event><small>只有本席先看到</small><strong>' + escapeHtml(control.localEvent) + "</strong></div>";
  }
  ...
  function operationsInstrument(control) {
    ...
    var final = state.phase === "FINAL" ? '<div class="signal warn">這裡不顯示林芮或支撐是否就位；聽兩席回報再拉閘。</div>' + meter("關閘進度", control.closeProgressExact, "%", control.closeProgressExact > 70 ? "warn" : "") + '<button type="button" class="hold-control danger-control" data-hold-start="CLOSE_START" data-hold-stop="CLOSE_STOP"' + (cooling ? " disabled" : "") + '>' + (cooling ? "拉桿回彈，機械冷卻中" : "按住拉下中央隔離閘") + '</button>' : "";
