...
const WORKER_ROOT = path.resolve(__dirname, "..", "..", "worker-a9r");
const STATIC_PORT = Number(process.env.MOMEY_A9R_STATIC_PORT || 1590);
const WORKER_PORT = Number(process.env.MOMEY_A9R_BROWSER_WORKER_PORT || 8790);
const REMOTE_FRONTEND = String(process.env.MOMEY_A9R_FRONTEND_URL || "").replace(/\/$/, "");
const REMOTE_WORKER = String(process.env.MOMEY_A9R_REMOTE_URL || "").replace(/\/$/, "");
const FRONTEND = REMOTE_FRONTEND || `http://127.0.0.1:${STATIC_PORT}`;
const WORKER = REMOTE_WORKER || `http://127.0.0.1:${WORKER_PORT}`;
const SCREENSHOTS = process.env.MOMEY_A9R_SCREENSHOT_DIR || "";
const EVIDENCE = process.env.MOMEY_A9R_FRONTEND_EVIDENCE || "";
const NPX_CLI = path.join(path.dirname(process.execPath), "node_modules", "npm", "bin", "npx-cli.js");
const BROWSER_EXECUTABLE = process.env.MOMEY_A9R_BROWSER || "C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe";
let staticServer;
let wrangler;
...
async function startServices() {
  ...
  throw new Error("A9R service health timeout");
}
...
async function phase(page, expected, timeout = 20000) {
  await page.waitForFunction((value) => window.__MOMEY_A9R__?.getState()?.phase === value, expected, { timeout });
}
...
async function clickPower(page, mode) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R__?.getState()?.control?.powerMode === value, mode, { timeout: REMOTE_WORKER ? 15000 : 5000 });
}
...
async function selectSeat(page, role) {
  ...
  await page.waitForFunction((value) => window.__MOMEY_A9R__?.getCurrentRole() === value && window.__MOMEY_A9R__?.getState()?.currentSeat?.started, role, { timeout: 10000 });
}
...
async function browserRun() {
  ...
  try {
    ...
    for (const beat of [1, 2, 3, 5, 6]) {
      await operations.waitForFunction((value) => window.__MOMEY_A9R__?.getState()?.briefing?.beat === value, beat, { timeout: 20000 });
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R__?.getState()?.training?.effects?.railPower === true);
    await snapshot(rescue, "09_training_rescue_receives_power.png");
    await train(rescue);
    await safety.waitForFunction(() => window.__MOMEY_A9R__?.getState()?.training?.effects?.gateLoad === true);
    await snapshot(safety, "10_training_safety_sees_pressure.png");
    await train(safety);
    await operations.waitForFunction(() => window.__MOMEY_A9R__?.getState()?.training?.effects?.support === true);
    ...
    const phaseBeforeReplay = await rescue.evaluate(() => window.__MOMEY_A9R__.getState().phase);
    await rescue.locator("[data-replay]").click();
    assert.equal(await rescue.evaluate(() => window.__MOMEY_A9R__.getState().phase), phaseBeforeReplay, "Replay must not advance shared state");
    ...
    await rescue.waitForFunction(() => window.__MOMEY_A9R__.getState().control.railPowered === false);
    await clickPower(operations, "rail");
    await rescue.waitForFunction(() => window.__MOMEY_A9R__.getState().control.railPowered === true);
    await down(safety, "[data-hold-start=BRACE_START]");
    await operations.waitForFunction(() => window.__MOMEY_A9R__.getState().control.safetySupportCoarse === "有人撐住");
    ...
    const outcome = await operations.evaluate(() => window.__MOMEY_A9R__.getState().outcome);
    ...
    console.log("A9R frontend three-device PASS " + JSON.stringify(report));
  } finally {
