(function (global) {
  ...
  var DIALOGUE = global.MOMEY_A9R_DIALOGUE || {};
  ...
  var workerBaseUrl = String(query.get("worker") || "https://momey-playable-a9r-room.momey-micro-mystery.workers.dev").replace(/\/$/, "");
  ...
  function storageKey(name) { return "momey-a9r:" + (roomCode || "none") + ":" + name; }
  ...
  function voiceMarkup(compactWhenDone) {
    ...
    return '<section class="voice" data-operator-expanded="true"><div class="caption-label">岬衛-7 字幕</div><p data-caption>' + escapeHtml(caption) + '</p><p class="audio-note">' + (master ? "此手機自動播放；它不是隊長。" : escapeHtml((state.audioMasterLabel || "另一支手機") + " 自動播放；字幕同步顯示。")) + '</p><button class="secondary" type="button" data-replay>重播</button></section>';
  }

  function briefingMarkup() {
    var briefing = state?.briefing || { beat: 0, total: 6, current: null };
    var current = briefing.current || {};
    return '<section class="stage briefing" data-view="briefing" data-briefing-beat="' + briefing.beat + '"><div class="briefing-progress"><span>共同事件簡報</span><strong>' + briefing.beat + ' / ' + briefing.total + '</strong></div><h1>' + escapeHtml(current.title || "海岬防洪站") + '</h1><img class="facility-map" src="assets/facility-map.svg" alt="海岬防洪站簡圖：西側救援軌道穿過中央隔離閘，林芮在西側，高承在閘門旁。"><div class="briefing-caption">' + voiceMarkup(false) + '</div><p class="briefing-sync">三支手機同步；播報結束會一起前進。</p>' + statusMarkup() + "</section>";
  }
  ...
  function trainingMarkup() {
    ...
    var received = currentRole === "rescue" && training.effects.railPower ? "現場調度已把測試電力送到你的軌道。" : currentRole === "safety" && training.effects.gateLoad ? "救援車吃電後，你的閘門負載已上升。" : currentRole === "operations" && training.effects.support ? "結構安全已接上支撐，你的控制台收到回應。" : "等待上一支手機的操作改變你的控制台。";
    ...
    return '<section class="stage" data-view="training"><div class="briefing-progress"><span>安全合作排演</span><strong>' + Math.min(training.step + 1, training.total) + ' / ' + training.total + '</strong></div><p class="eyebrow">' + escapeHtml(ROLE_LABELS[currentRole] || "控制席") + '</p><h1>' + escapeHtml(waiting) + '</h1><div class="training-effect ' + (received.indexOf("等待") === 0 ? "is-waiting" : "is-received") + '" data-training-effect>' + escapeHtml(received) + '</div><div class="instrument training-instrument">' + control + '</div><div class="event-strip">依序排演：切電 → 推進 → 支撐</div>' + statusMarkup() + "</section>";
  }
